...
///////////////////////// ADDED BY MO: add resultBundlePath option to capability. it is reloated to xcodebuild failure: xcodebuild failed with code 65 issue.  ///////////////////
...
///////////////////////// ADDED BY MO: add resultBundlePath option to capability. it is reloated to xcodebuild failure: xcodebuild failed with code 65 issue.  ///////////////////
    if (this.resultBundlePath) {
        log.debug(`Using Xcode resultBundlePath : '${this.resultBundlePath}'`);
...
///////////////////////// ADDED BY MO: add resultBundlePath option to capability. it is reloated to xcodebuild failure: xcodebuild failed with code 65 issue.  ///////////////////
    try {
        if (this.resultBundlePath && await fs.exists(this.resultBundlePath)) {
            log.debug(`The resultBundlePath is existed. Cleanup resultBundlePath ${this.resultBundlePath}`);
            await fs.rimraf(this.resultBundlePath);
        }
    } catch (e) {
        log.error(e);
    }
